...
if [[ $(docker -H unix:///var/run/early-docker.sock inspect blacksmith) == "[]" ]]; then
  ...
  ARGS="-etcd http://<< (cluster_variable "bootstrapper1_ip") >>:2379,http://<< (cluster_variable "bootstrapper2_ip") >>:2379,http://<< (cluster_variable "bootstrapper3_ip") >>:2379 -if $1 -cluster-name << (V "cluster_name") >> -lease-start << (V "internal_network_workers_start") >> -lease-range (cluster_variable "internal_network_workers_limit") -dns << (cluster_variable "external_dns") -file-server (cluster_variable "file_server") -workspace-repo git@git.cafebazaar.ir:ali.javadi/blacksmith-kubernetes.git >>"
